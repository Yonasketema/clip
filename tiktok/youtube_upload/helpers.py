...
class YouTubeUploader:
    ...
    def __init__(self, video_path: str, metadata_json_path: Optional[str] = None,
        ...
        self.browser = Firefox(profile_path=profile_path,
                               pickle_cookies=True, full_screen=False)
        ...
        self.logger.debug("Use profile path: {}".format(
            self.browser.source_profile_path))
    ...
    def __login(self):
        ...
        if self.browser.has_cookies_for_current_website():
            ...
            self.logger.debug("Loaded cookies from {}".format(
            ...
            self.logger.debug("Saved cookies to {}".format(
                self.browser.cookies_folder_path))
        ...
        try:
            if self.thumbnail_path is not None:
                absolute_thumbnail_path = str(
                    Path.cwd().parent / self.thumbnail_path)
            ...
            if video_description:
                self.__write_in_field(
                    description_field, video_description, select_all=True)
            ...
            if playlist:
                ...
                time.sleep(Constant.USER_WAITING_TIME)
                search_field = self.browser.find(
                    By.ID, Constant.PL_SEARCH_INPUT_ID)
            ...
            if tags:
                tags_container = self.browser.find(
                    By.ID, Constant.TAGS_CONTAINER_ID)
            ...
            self.browser.find(
                By.XPATH, Constant.PUBLISH_TO_SUBSCRIPTIONS_TOGGLE).click()
            ...
            if schedule:
                upload_time_object = datetime.strptime(
                    schedule, "%m/%d/%Y, %H:%M")
                self.browser.find(
                ...
                public_main_button = self.browser.find(
                    By.NAME, Constant.PUBLIC_BUTTON)
                self.browser.find(By.ID, Constant.RADIO_LABEL,
                                  public_main_button).click()
                self.logger.debug(
                    'Made the video {}'.format(Constant.PUBLIC_BUTTON))
            ...
            while uploading_status_container is not None:
                uploading_progress = uploading_status_container.get_attribute(
                    'value')
                self.logger.debug(
                    'Upload video progress: {}%'.format(uploading_progress))
            ...
            if done_button.get_attribute('aria-disabled') == 'true':
                error_message = self.browser.find(
            ...
            print("Error occured, video upload limit may be reached")
            return False, None
