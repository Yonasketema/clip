...
from tiktokUploader.tk_config import Tk_Locator
from config.base_social_media import set_init_script
from config.log import tiktok_logger

from config.files_times import get_absolute_path
...
async def tiktok_setup(account_file, handle=False):
    account_file = get_absolute_path(account_file, "tiktokUploader")
...
class TiktokVideo(object):
    ...
    async def set_schedule_time(self, page, publish_date):
        ...
        #             'div.calendar-wrapper span.arrow').nth(-1)
        #     else:
        #         arrow = self.locator_base.locator(
        #             'div.calendar-wrapper span.arrow').nth(0)
        #     await arrow.click()
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        try:
            ...
            tiktok_logger.error(
                "Neither iframe nor div appeared within the timeout.")
        ...
        if self.thumbnail_path:
            tiktok_logger.info(
                f'[+] Uploading thumbnail file {self.title}.png')
    ...
    async def add_title_tags(self, page):
        ...
        editor_locator = self.locator_base.locator(
